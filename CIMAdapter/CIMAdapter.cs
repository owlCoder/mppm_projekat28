	...
	public class CIMAdapter
	{
		...
		private bool DoTransformAndLoad(Assembly assembly, ConcreteModel concreteModel, SupportedProfiles extractType, out Delta nmsDelta, out string log)
		{
			...
			try
			{
				...
				switch (extractType)
				{
					case SupportedProfiles.Projekat28:
						{
							// transformation to DMS delta					
							TransformAndLoadReport report = Projekat28Importer.Instance.CreateNMSDelta(concreteModel);

							if (report.Success)
							{
								nmsDelta = Projekat28Importer.Instance.NMSDelta;
								success = true;
							}
							else
							{
							...
							Projekat28Importer.Instance.Reset();
